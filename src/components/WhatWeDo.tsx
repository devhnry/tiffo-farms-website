...
export const WhatWeDo = () => {
  ...
  const data = [{
    ...
    desc: "From aquaculture and poultry to crop cultivation, we run a model that combines nature’s cycles with smart management for maximum productivity"
  },{
    ...
    desc: "By recycling by-products, promoting local sourcing, and reducing waste, we protect the environment while supporting food security"
  }]
  ...
  return (
    <section className={`max-w-[720px] xl:max-w-[1100px] mx-auto w-full pb-16 lg:pb-24`}>
      ...
      <div className={`bg-[#F5F5F5] border-white border-[1.5px] font-medium shadow-fancy-two rounded-2xl p-3 flex flex-col gap-4 my-8 2md:grid 2md:grid-cols-2 md:gap-2`}>
        {data.map((item, index) => (
          <div 
            key={index} 
            className={`
